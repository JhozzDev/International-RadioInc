...
useEffect(() => {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-[3%] h-14">
        <button
        ...
        <button
          onClick={() => { console.log("click países"); setProfileOpen(true); setMenuOpen(false); }}
          className="bg-purple-700 text-white border-none rounded-xl px-4 py-2 text-base cursor-pointer shadow-[0_0_20px_#7c3aed] hover:bg-purple-600 transition-colors"
        >
          👤 Perfil
        </button>
      ...
      >
        ...
        <div className="flex flex-col p-3 gap-1">
          {["Mi cuenta", "Favoritos", "Historial", "Configuración", "Idioma", "Notificaciones", "Privacidad", "Ayuda", "Acerca de", "Cerrar sesión"].map(
            (item) => (
              <div
                key={item}
      ...
      {/* Player card */}
      <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 text-center text-white w-[340px] mx-4">
        {/* Cover */}
        ...
        {/* Status & counter */}
        <p className="text-sm text-gray-400">{loading ? "⏳ Cargando..." : estado}</p>
        {radios.length > 0 && (
